 ...
 */
class IconProvider implements ResourceProvider {

    public Resource get(final ResourceRequest request) {
        return ResourceCustom.copyWithSize(ServiceLocator.getPersistance().getIcon(request.getId()),
                ResourceSizeCustom.createByWidthHeight(Icon.getDefaultWidth(), Icon.getDefaultHeight()));
    }
