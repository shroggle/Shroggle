 ...
 * @author Artem Stasuk
 */
public class ResourceResizerHighQuality implements ResourceResizer {
    ...
    public void execute(
        ...
        if ("gif".equals(extension.toLowerCase().trim()) || "ico".equals(extension.toLowerCase().trim())) {
            try {
        ...
        try {
            ...
            if (destinationSize.isCrop()) {
                g2.drawImage(sourceData.get(), 0, 0, null);
            } else {
                final Image scaledImage = sourceData.get().getScaledInstance(dimension.getWidth(), dimension.getHeight(), Image.SCALE_AREA_AVERAGING);
            ...
            throw new ResourceResizeException("Can't resize with ext: " + extension, exception);
        }
    }
