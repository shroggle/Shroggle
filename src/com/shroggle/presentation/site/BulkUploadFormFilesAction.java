 ...
 */
@UrlBinding("/bulkUploadFormFiles.action")
public class BulkUploadFormFilesAction extends Action implements UploadedFiles {
    ...
    public Resolution upload() {
        try {
            ...
            final SynchronizeRequest synchronizeRequest = new SynchronizeRequestEntity(FilledFormItem.class,
                    SynchronizeMethod.WRITE, filledFormItem.getItemId());
            ServiceLocator.getSynchronize().execute(synchronizeRequest, new SynchronizeContext<Void>() {
                public Void execute() {

                    final Integer formImageId = new FilledFormItemManager(filledFormItem).getFormImageId();
                    final FilledForm copiedFilledForm;
                    if (formImageId == null || formImageId <= 0) {
                        copiedFilledForm = filledForm;
                    } else {
                        copiedFilledForm = persistanceTransaction.execute(new PersistanceTransactionContext<FilledForm>() {
                            @Override
                            public FilledForm execute() {
                                return new FilledFormManager(filledForm).copyFilledForm();
                            }
                        });
                    }
                    persistanceTransaction.execute(new Runnable() {
                        public void run() {
                            final DraftForm form = persistance.getFormById(copiedFilledForm.getFormId());
                            final FilledFormItem filledFormItem = FilledFormManager.getFilledFormItemByItemPosition(copiedFilledForm, position);
    ...
    private static final Logger logger = Logger.getLogger(BulkUploadFormFilesAction.class.getName());
